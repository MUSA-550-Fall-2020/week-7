# Week 7<br>Analyzing and Visualizing Large Datasets

**Note:** Binder has **very** limited memory resources, so these large datasets won't work — you'll need to run the notebook on your local laptop.

## New Packages

- [intake](https://intake.readthedocs.io)
- [dask](http://docs.dask.org/en/latest/)
- [datashader](http://datashader.org/index.html)

## Useful Links and Reference Materials

- [holoViz Large Data Tutorial](https://holoviz.org/tutorial/Large_Data.html)
- [Datashader User Guide](http://datashader.org/user_guide/index.html)
- [Datashader Use Cases](http://datashader.org/topics/index.html)
